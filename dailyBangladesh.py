...
newspaper_base_url = 'https://www.daily-bangladesh.com/'

for index in range( 4564 , 4565 ):
    with open("log.txt","a") as logFile:
        logFile.write(str(index) + "\n")
        url = newspaper_base_url + 'archives/' + str(index) + '/article'
        try:
            print(url)
        ...
        if(page_links_length == 0):
            ...
            for link in all_links:
                ...
                try:
                    author = article_soup.find("span",{"class":"DAuthor"}).get_text().strip()
                except:
                    author = ""

                try:
                    date = article_soup.find("div",{"class":"row DDateHit"}).get_text().strip()
                except:
                    date = ""

                try:
                    article_content = article_soup.find("div",{"id":"DDetailsBody"}).get_text().strip()
                except:
                    article_content = ""

...
for i in range(delta.days + 1):
    ...
    for i in range(1,10):
        url = newspaper_archive_base_url + "/" + str(date_str.year) + "/" + str(date_str.month) + "/" + str(date_str.day) + "/page/" + str(i)
        try:
            print(url)
        ...
        if(page_links_length == 0):
            ...
            for link in all_links:
                ...
                try:
                    article_body_text = article_soup.find("div", {"class": "entry-content"}).get_text().replace("printশেয়ার করুনTweetEmail","").strip()
                except:
                    article_body_text = ""

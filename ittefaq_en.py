...
newspaper_base_url = 'https://www.clickittefaq.com/'
...
for i in range( 0 , lim1 ):
    ...
    for j in range(max_category):
        ...
        if(j==1):
            if(max_politics_pages>=i+1 ): url = newspaper_base_url + "politics/page/" + str(i+1)
            else: url = "none" 
        if(j==2):
            if(max_international_pages>=i+1): url = newspaper_base_url + "international/page/" + str(i+1)
            else: url = "none"
        if(j==3):
            if(max_business_pages>=i+1): url = newspaper_base_url + "business/page/" + str(i+1)
            else: url = "none"
        if(j==4):
            if(max_editorial_oped_pages>=i+1): url = newspaper_base_url + "editorial-oped/page/" + str(i+1)
            else: url = "none"
        if(j==5):
            if(max_sports_pages>=i+1): url = newspaper_base_url + "sports/page/" + str(i+1)
            else: url = "none"
        if(j==6):
            if(max_sci_tech_pages>=i+1): url = newspaper_base_url + "sci-tech/page/" + str(i+1)
            else: url = "none"
        if(j==7):
            if(max_culture_pages>=i+1): url = newspaper_base_url + "culture/page/" + str(i+1)
            else: url = "none"
        if(not("none" in url)):
            print('\n'+ url)
            ...
            if(page_links_length == 0):
                ...
                for link in all_links:
                    ...
                    output_file_name = url_parts[3] + '_' + url_parts[4] + '_' + str(i+1) + '_' + txtFiletitle + "_abc.txt"
                    try:
